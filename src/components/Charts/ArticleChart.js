...
const ArticleChart = () => {
  const [series, setSeries] = useState([{ name: "Total Artikel", data: [] }]);
  ...
  const fetchData = async (time) => {
    try {
      const response = await axios.get(`admin/article-chart?time=${time}`);
      ...
      setCategories(dates);
      setSeries([{ name: "Total Artikel", data: counts }]);
    } catch (error) {
        ...
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-primary">Total Artikel</p>
            </div>
          </div>
        </div>
